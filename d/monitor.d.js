...
export async function main(ns) {
    ...
    const program = async () => {
        ns.clearLog()
        // Find all .d files
        const daemons = ns.ls('home', '.d')
        // print a table showing whether or not each is running
        const table = new utils.TableHelper(utils, ['Program', 'Status'])
        daemons.forEach(x => table.data.push([x, ns.scriptRunning(x, 'home')]))
        table.print()
    }
